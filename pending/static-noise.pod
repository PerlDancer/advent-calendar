...
following steps, run in the following order:
...
handlers. L<Dancer2::Handler::File> was to serve static files (which
...
In order for these routes to accept I<any> input, they used a greedy
...
sequence of a matched route, allowing you to call code before it is executed
...
I<"You can't see the picture for lacking access because you lack access!">

=head2 Plack strikes again

Before we hit a I<Strike three> we decided to fix this. Since we're
depending on Plack and the associated technology to help resolve so many
...
is no longer in charge of serving static files anymore.
