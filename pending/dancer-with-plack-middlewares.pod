...
not yet familiar with L<Plack>'s middleware, you can refer to these articles:
...
then store this value, and next time it requests the same page, he will ask to
...
L<Plack::Middleware::Deflater> that will compress the content on the fly to
gzip if the client support this feature.
...
One very useful middleware while debugging your application is
L<Plack::Middleware::Debug> and L<Dancer::Debug>. Those middlewares will inject some HTML code in your
page. To activate the panels, add this to your configuration:
