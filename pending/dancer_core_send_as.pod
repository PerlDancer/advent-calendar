...
However if you have a small number of routes that required a templat engine
...
Dancer2's C<send_file> and C<to_json> using an in-memory file handle:
  ...
  get '/html/route' => sub {
      my $html = encode( 'UTF-8', template('awesome_template.tt') );
...
serializer is defined:
   ...
   post '/api/:id' => sub {
       ...
       if ( $id !~ m/^[0-9]+$/ ) {
           ...
           status 500;
           send_as HTML => template('error.tt');
       }
