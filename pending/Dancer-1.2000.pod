=head1 Dancer 1.2000 - a micro web framework milestone
...
that exists in web programming and let's you focus just on the important
...
This shows off a an HTTP B<GET> request route with a variable in it which is
attached to an anonymous subroutine which gets the ID and renders a template
(with supported inlined layout - even if not supported in your template engine
of choice) with variables. It really is that simple.
...
=head2 Dancer 1.2000 is finally out

We've been speaking our heads off on the B<1.2000> release of Dancer for a while
now. Why are we excited so much, you say? Well, for a few reasons!

=head3 The community blossomed

The Dance community has grown with time and provided a nice warm place for
people to raise questions, ideas and create things together. Version 1.2000
...
decided it's was about time to implement it.
...
policy. When Dancer 1.2000 finds you're doing something deprecated, it will not
...
web applications dread. Since Dancer itself supports PSGI/Plack, deploying it is
...
Therefore, we put effort in preparing L<Dancer::Deployment> which carries out
...
While we hope you enjoy version 1.2000, we're not about to rest on our laurels.
...
your apps on various backends and understand the beauty of simplicity that
Dancer offers using its clean DSL and route-based structure.

Stay tunes for more awesomeness!

=head2 Credits

Credit goes to the entire Dancer community for our 1.2000 release and the advent
calendar. Dancer would definitely not be where its at right now without the love
