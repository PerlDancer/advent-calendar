...
Fetching the informations from mongo and rendering them can be really slow, depending on the size of the requestd graph. A good idea is to cache the result before sending it back to the client. For this, we can use B<Dancer::Plugin::Memcached>.
