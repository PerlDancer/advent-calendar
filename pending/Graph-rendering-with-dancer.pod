...
This is not the kind of graph I want to end up with (this one is buggy, some profiles are twice on the graph etc), but it gives you a good idea of what I want to achieve. Check the content of the iframe to see the html and javascript code.

=head2 The data

I'm collecting data using the GitHub API, and storing most of them in L<mongodb|http://www.mongodb.net/> and using the L<Dancer::Plugin::Mongo|http://search.cpan.org/perldoc?Dancer::Plugin::Mongo> plugin. I've got two collections:
...
I set the layout to undef since I only want only the graph and nothing else. 
...
Here I use B<Dancer::Plugin::Mongo>. It imports the B<mongo> keyword; B<github> is the name of my database; B<relations> is the name of the collection in the database. To finish, I call the B<find> method with my query, to fetch the informations I need.

The B<_add_links> function is similar. Now that we have all our informations, we can ask for a list of nodes and edges, and return them to the javascript.

For the graph rendering, I use the amazingly great L<protovis library|http://vis.stanford.edu/protovis/>.

=head2 Caching

Fetching the informations from MongoDB and generating the graph object can be really slow, depending on the size of the requestd graph. A good idea is to cache the result before sending it back to the client. For this, we can use L<Dancer::Plugin::Memcached|http://search.cpan.org/perldoc?Dancer::Plugin::Memcached>.
...
You can see the introduction of two new keywords: B<memcached_get> and B<memcached_set>. 

=head2 Conclusion (bis)

In this article I've show two new plugins: B<Dancer::Plugin::Mongo> (by Adam Taylor) and B<Dancer::Plugin::Memcached> (by squeeks), and a nice javascript library : protovis.

I'll continue to work on this app in the following month, and you can expect to see the code (and use the application) soon.
