...
As a web framework, Dancer2 provides you with many keywords you can use to write your web code. Usually, this presents no problem, but in some cases Dancer2 might give you a function with the same name as a function in another module you wish to use. For example, Dancer2 provides a function C<any>, so you can write endpoints that work on multiple HTTP verbs, while L<List::Util> provides C<any>, a function which returns a true value if at least one item in a list matches a condition.

If you want to use L<List::Util/any> in a package where Dancer2 has been C<use>d, you will find you cannot import both at once. Instead, you can qualify the C<any> from L<List::Util>:

  any '/page/:id' => sub {
    my $id = param('id');
    send_error("Blocked!", 401) if List::Util::any { $_ eq $id } @blocked_resources;
    template 'page', { id => $id };
  };
...
However, you cannot selectively import functions from Dancer2, and you cannot use the qualified form C<Dancer2::any>, because there isn't actually an C<any> function in the C<Dancer2> package.

The reason for this is that Dancer2's C<import> function (which is called when you C<use> it) does something clever, so that you can have multiple separate Dancer2 apps running in the same perl instance.

  use Dancer2 appname => 'MyApp';

NB: See L<http://advent.perldancer.org/2014/10> for more on C<appname> and running multiple apps.
...
Don't forget, it's not just Dancer2's functions that you can put in your DSL package. If you have other functions you use frequently and want to make available across your whole app (such as L<Scalar::Util/blessed>), you can add them too.
...
One strategy therefore is to create a different tag for each group of concerns, e.g. C<:controller>, C<:model>, C<:route>, etc. Alternatively, the tags could represent where the functions come from, e.g. C<:util>, C<:config>, C<:storage>, C<:routing>, etc.
...
© 2016 Daniel Perrett; you are free to reuse code without restriction and text under the L<Creative Commons Attribution-ShareAlike License 4.0|http://creativecommons.org/licenses/by-sa/4.0/>.
