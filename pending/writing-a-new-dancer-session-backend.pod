...
L<Dancer::Session::Cookie> has a plus side of being encrypted also.
...
want, but we also want the user not to have to do it so we're going to allow a
default value.
...
etc.) and we've allows basic recursive folding of our object using
I<allow_classes>.

B<Important:> you should know that we've being very naive here by not providing
...
KiokuDB this method can't be any easier:
...
This method deletes the session object completely. Again, using Kioku, it's just
too easy.
