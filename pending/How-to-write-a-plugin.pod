...
For advanced or particular needs, Dancer'DSL can be extended by plugins.
...
bound to I<code refs>. When a plugin has done all of its declarations,
...
just like if they were part of Dancer's core DSL.

A plugin can also do all what a regular Dancer application does, like
declarign a before filter, declaring a couple of route handlers or
...
will provide the developer with a facility to detect if the user agent
