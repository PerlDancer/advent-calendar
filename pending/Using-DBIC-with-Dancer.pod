...
B<Note> : Although this article only skimm the surface of C<DBIX::Class>, it
won't explain what C<DBIC> is or how to use it. We reommand you have a look at
...
will be a bit simple. However, we'll try to explain how to properly use
...
And thatt's our database populated !
...
your data are organized in your database. There are two ways of letting DBIC
know:
...
doesn't get everything right, so one needs to help DBIC. describing
the schema manually seems a cleaner option. But hey, TIMTOWTDI.
...
And now we can implement C<_perform_search> using
C<Dancer::Plugin::DBIC>. It's super easy: the plugin gives you access to an
additional keyword C<schema>, which you give the name of schema you want to
retrieve. It returns a C<DBIx::Class::Schema::Loader> (because we let the
plugin discover the schema for us). This returned object can then be used to
get a resultset and perform searches, as per standard usage of C<DBIX::Class>.
...
authors. Now if e search for C<The>, here is what we get :
...
Or you can use the nice C<DBIx::Class::MooseColumns> that let's you write the
DBIC schema classes using C<Moose>. This way of doing make it look more like
ActiveRecord declaration.
