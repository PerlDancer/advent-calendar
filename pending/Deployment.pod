=head1 Deploying Dancer - VERY EARLY DRAFT QUALITY
...
Note #1: L<Plack::Runner> is what will actually be providing the CGI environment to Apache. 
Note #2: If L<Task::Dancer> fails for the hosting company, request individuals modules instead.
...
hosting company set the Apache defaults properly.
...
then a rewrite rule is rarely necessary. Regardless here's a simple rewrite rule: 
   ...
   RewriteRule ^myapp/public/index.cgi/(.*)  /$1 [L,R=301]
...
=over 4
=item 1. Permissions - Does the B<webserver user> have access to read, and possibly execute, the file?
=item 2. 'public/' - Is the file inside '/public'? It shoud work otherwise check your config.yml paths carefully.
=item 3. Absolute Path - If you plan to use files outside of 'public/' make sure the path is B<system absolute>.
=over 2
=item a. The environment that your application runs under and the one your account uses are usually different
=item b. '/home/public', '/home/htdocs', ect.. are NOT absolute paths but set by user environments usually. *  
=back
=back
...
Example (/etc/nginx)/nginx.conf:
 ...
 events {
   ...
   # There are 7 different polling methods nginx can use. Here are recommended for each OS though
 ...
 http {
   ...
   # 90% of the time you don't have to touch it.
...
Example (/etc/nginx)/sites-available/default:
 ...
 server {
	 ...
	 # Port 80 is implied but with a caching server, you need different port if caching server is in front. 
	 #listen        80;
         server_name   mydomain.com;
	 
	 # It is recommended you create a separate access log for 
	 #access_log ;

	 # This is different access log directly relating to  
	 
	 # Serve static files using nginx thus allowing Dancer to handle more dynamic content requests.
	 # B<Huge> performance boost! First you must move all your static ('/public') folder to the same
...
B<Note:> Using HTTP::Server::PSGI is B<NOT> a good idea for production. Just using it as an example.

B<Note #2:> Sometimes the -D option in plackup doesn't properly daemonize the server since it's the 
...
B<Note #3:> It's probably a good idea to create a start up script that launches you servers and 
in the background.

=head2 Caching

Caching it up to you and your personal preferences. Usually it's best to use URL matching. I suggest
putting a caching server in front of nginx such as Varnish or Squid. The request flow should be:
...
nmani: Naveen Manivannan <nmani@nashresearch.com>