...
=head3 Handle multiple sources

Normally, you would dictate to a user how they should send their
...
an old interface or supporting outdated APIs.

L<Dancer2::Plugin::ParamTypes> is flexible enough to support multiple
sources for an argument:
...
In this form, the parameter C<format> can be provided either in the
query string or in the body, because your route might be either a
B<GET> or a B<POST>.

=head3 Register type actions
