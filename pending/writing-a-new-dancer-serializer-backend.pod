...
Writing serializers in L<Dancer> is so easy you'll can read this standing on
your head!

In this entry we'll be writing a new serializer for Dancer and upload it to
CPAN! Feel free to add your owns once you finish reading this. :)
...
=item * Deserialize a data structure

Take a serialized data as a parameter and return structured data.

=back

=head2 Deciding on the serializer
...
And since the recommended serializers are already mentioned above, it's a good
reason to look into more nether regions, such as... UU encoding!

Of course, UU encoding isn't a serializer, just an encoding so it isn't
...
subroutines functionally outside of automatic serialization. This is common for
...
I<Writing a new Dancer logger backend>
