...
The core is all that Dancer needs to regsiter route handlers and dispatch any
...
(yes, you can have multiple apps within the same application). An application
object provides a route registry, where we can find L<Dancer::Route> objects,
that represents the route handlers.
...
L<Ovid|http://blogs.perl.org/user/Ovid/> explained on his blog L<the concept of the abstract-class pattern|http://blogs.perl.org/users/ovid/2010/11/perl101-encapsulation-via-protected-abstract-methods.html>, the article is part of his Perl 101 serie and explained very well the concept.
...
For instance, writing a new template engine would bethat simple:
...
backends available on the CPAN. This shows well, I think, one of the idea
behind Dancer's philosophy: being as open as possible for the developer and for
the Perl community.
...
Finally, we have plugins. As explained in a previous artile, plugins are the
most simple way to extend Dancer on a particular need. If you plan to
contribute code to Dancer, your probably want to consider writing a plugin.
