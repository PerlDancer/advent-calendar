=head1 New feature in Dancer2: appname
...
A PSGI application is a code reference which receives a request as an env
hash (from a server using a L<Plack> handler.

=head2 Too many apps!

As soon as you import L<Dancer2> (C<use Dancer2>), you have just created a
Dancer2 App. It will use your package (defined by either the C<package>
function in Perl or the default in Perl: C<main>) in order to define the App
...
one), C<MyApp::User::Edit> does not.
...
mix this with the C<to_app> method, you have a problem:
...
I<<(There is another article on C<to_app> specifically.)>>
...
help it just extend it, that is not the case. It will still fail.
...
will even interally just represent these with a single application object.
...
Usually we would put extensions such as these inside the class extended, as
such:
...
This allows you to spread your application routes on multiple files and allow
