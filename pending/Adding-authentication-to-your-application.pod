...
security models (e.g. ACL, etc), Dancer::Plugin::Authorize provides flexible and 
...
RBAC works, and finally, show you a few examples on how Dancer::Plugin::Authorize
approaches RBAC. 
...
Dancer::Plugin::Authorize only ships with one access control module which is 
Dancer::Plugin::Authorize::Permissions::Config, which should be sufficient for most 
use cases.

A typical web application using Dancer::Plugin::Authorize should load the 
Dancer::Plugin::Authorize plugin and specify one authentication module and one 
access control module. Dancer::Plugin::Authorize will then provide all the neccessary
...
Dancer::Plugin::Authorize. First we need to decide how we will be creating and 
...
Dancer::Plugin::Authorize::Permissions::Config, since there is only one access 
...
provided by Dancer::Plugin::Authorize. Dancer::Plugin::Authorize provides the auth()
function to Dancer which returns a new instance of Dancer::Plugin::Authorize which 
