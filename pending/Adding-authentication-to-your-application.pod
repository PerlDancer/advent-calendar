...
authenticate them, and L<Dancer::Plugin::Authorize::Permissions> which specifics
...
login page, e.g.
    ...
    before sub {
        ...
        return redirect '/login' unless session('user')->{'id'};
    };
...
Even better, use RBAC in you TT (Template-Toolkit) templates as follows:
...
th best web application framework with minimal effort. Let keep web application development
fun! Thanks for reading.

=head1 Author

Al Newkirk, C<< ime@ana.im> >>
