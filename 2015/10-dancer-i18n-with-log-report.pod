...
strings, see L<Log::Report::Extract::PerlPPI>.

For the purposes of this exercise, a PO file should have been created
containing with the 2 message IDs ("Hello world" and "Hello {name}"). From this
default PO file, create a specific language file in a new directory for this
domain:

 mkdir messages/default/
 cp messages/default.utf-8.po messages/default/de_DE.utf8.po

Now edit the file, inserting the translation text:
...
template. Add the following to your route handler:
 ...
 sub translate {
   my $msg = Log::Report::Message->fromTemplateToolkit('default', @_);
   $msg->toString;
 }
...
template. As a simple example, add the following to your index template:
...
in a similar manner to last time, but this time specifying template files:
 ...
 xgettext-perl -p messages --from-code=iso-8859-1 --template TT2-loc --domain default .

Note that this will overwrite your original extractions. Add the newly
extracted message ID and its translation to your previous PO file:
