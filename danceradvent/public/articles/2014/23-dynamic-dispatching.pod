...
of a single forward slash, as the examples above do, such as:

    prefix '/:board' => sub {
        get '/' => sub {...};
    };

Both of these are fixed in 0.158000. The above examples would work without
this code on an older version, but to make sure copy-pasted examples work as
is without changes, you would need to version indicated.
