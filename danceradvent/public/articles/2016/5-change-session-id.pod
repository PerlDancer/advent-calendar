...
It is considered good security practice to change session ID on any change
of privilege level, for example on login.  It is also a requirement of a
number of established security standards so is something you should be
doing in your web applications.

=head2 Session destruction

Destroying a session in L<Dancer2> has always been possible using
L<Dancer2::Core::App/destroy_session> thus:
...
If you are planning to write a new session engine you should really read
this part...
...
which has the signature:
    ...
    _change_id $self, $old_id, $new_id

Where C<$self> is the session engine object, and C<$old_id> and C<$new_id>
