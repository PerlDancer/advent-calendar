...
Plack is an awesome addition to the Perl ecosystem. One of it's key feature is
the middlewares.
...
is a checksum of the page's content returned to the client. The client will
...
L<Plack::Middleware::Deflater> that will compress the content on-the-fly with
