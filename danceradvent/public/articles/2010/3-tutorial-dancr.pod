=head1 Tutorial: writing a tiny blog engine

This article will guide you through the writing of a minimal but yet
complete blog engine.

=head2 Origins of this tutorial

While I was investigating some Python
web frameworks like L<Flask|http://flask.pocoo.org/> or
...
more involved than a trivial example.
...
worked. (I'm learning it too!)

So "Dancr" was born.

Dancr is a simple "micro" blog which uses the L<SQLite|http://www.sqlite.org>
database engine for simplicity's sake.

=head2 Required Perl modules

Obviously you need L<Dancer>.  You also need L<Template Toolkit|Template>,
L<File::Slurp> and L<DBD::SQLite>. These all can be installed using your CPAN
client, as in:
...
point of this particular tutorial.
...
Here we have a single table with three columns: id, title, and text.  The 'id'
field is the primary key and will automatically get an ID assigned by the
database engine when a row is inserted. 

We want our application to initialize the database automatically for us when we
start it, so open your favorite L<text editor|http://www.vim.org> and create a
...
Let's tackle our first route handler now, the one for the root URL '/'. This is
what it looks like:
...
ask why we don't simply hard-code the C</add> URI in our application or
...
here for always, always using parameterized INSERTs in your application's SQL
...
If you want to test the application of this tutorial, you can grab the complete
source code on
L<GitHub|https://github.com/PerlDancer/advent-calendar-apps/tree/master/2010/03-dancr/>

    git clone https://github.com/PerlDancer/advent-calendar-apps.git
    cd advent-calendar-apps/2010/03-dancr/
    perl dancr.pl

=head2 Advanced route moves
...
Calendar. It's also shipped with Dancer as L<Dancer::Tutorial>.
