=head1 Asynchronous Dancer2 using Promises PT. 1 - The What and Why
...
L<this article|https://advent.perldancer.org/2020>
...
=head2 So what example are w using here?
...
First, our initial code:
    ...
    use constant {
        'MAX_COUNTRIES' => 5,
        'MAX_DAYS'      => 7,
        'SUMMARY_URL'   => 'https://covid-api.mmediagroup.fr/v1/history?status=confirmed',
...
we need to change the C<sub> into a C<delayed> sub.

=back

Using C<delayed> can be done without parenthesis (just like other
Dancer2 keywords) and without the C<sub> keyword. Or, if you decide
to use it with parenthesis and the C<sub> keyword, you could use
parenthesis.
...
Promises is such a popular pattern, which we will see in the next article.
...
Once we these list of top countries, we want to get the results for each
country's history. Here is where the real magic happens.

We want to make another C<http_get> call for each country.
...
The statistics are not necessarily helpful or even accurate. You might come
up with a better calculation that is more accurate and provides more insight.
