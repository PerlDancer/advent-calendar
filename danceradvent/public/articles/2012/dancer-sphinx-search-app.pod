...
This creates a new instance of Sphinx::Search (which will be used to do the
searches), and sets up a few basic options, such as how many results should be
returned and in what order. Now comes the most interesting part -- actually
performing a search in our application. We insert this chunk of code at the
beginning of the C<get '/'> route handler:
    ...
    if (my $phrase = params('query')->{'phrase'}) {
        ...
        if ($total_count = $results->{'total_found'}) {
            ...
            # Join the IDs to use in SQL query
...
=for html <img src="/images/2012/FIXME/search-results.png">
